...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        self.train_iterator = get_batch_iterator(**data_iterator_kwargs, split='train',shuffle=True, n_epochs=config.n_epochs, n_examples=config.n_examples, batch_size=config.batch_size, silent=rank != 0, cache_dir=get_local_dir(config.local_dirs), sft_mode=config.loss.name == 'sft')
        rank0_print(f'Loaded train data iterator')
        self.eval_iterator = get_batch_iterator(**data_iterator_kwargs, split='test', shuffle=False, n_examples=config.n_eval_examples, batch_size=config.eval_batch_size, silent=rank != 0, cache_dir=get_local_dir(config.local_dirs), sft_mode=False)
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        if loss_config.name in {'dpo', 'ipo'}:
            ...
            # decode = pad_to_length(batch['chosen_input_ids'], self.config.max_length, self.tokenizer.pad_token_id)
            # decode = all_gather_if_needed(decode, self.rank, self.world_size)
            # decoded = self.tokenizer.batch_decode(decode, skip_special_tokens=True)

            # print("chosen input ids", batch['chosen_input_ids'], decoded)
            policy_chosen_logits = self.policy(batch['chosen_input_ids'], attention_mask=batch['chosen_attention_mask']).logits.to(torch.float32)
            policy_chosen_logps = _get_batch_logps(policy_chosen_logits, batch['chosen_labels'], average_log_prob=False)
        ...
        if "rejected_input_ids" in batch:
            policy_chosen_logps, policy_rejected_logps = self.concatenated_forward(self.policy, batch)
            classify_accuracy = (policy_chosen_logps > policy_rejected_logps).float()
            classify_accuracy = all_gather_if_needed(classify_accuracy, self.rank, self.world_size)
            metrics[f'classify_accuracy_{train_test}'] = classify_accuracy.cpu().numpy().tolist()
    ...
    def train(self):
        ...
        def eval():
            ...
            if self.config.sample_during_eval:
                ...
                policy_text_table = wandb.Table(columns=["step", "prompt", "sample", "answer"])
                if self.config.loss.name in {'dpo', 'ipo'}:
                    reference_text_table = wandb.Table(columns=["step", "prompt", "sample", "answer"])
            ...
            if self.config.sample_during_eval:
                ...
                for eval_batch in (tqdm.tqdm(sample_batches, desc='Generating samples...') if self.rank == 0 else sample_batches):
                    ...
                    # TODO: check if policy samples contain correct answer
                    num_correct = 0
                    word_count = 0
                    for i, sample in enumerate(policy_samples):
                        sample_only = dataset_utils.extract_sample(sample)
                        word_count += len(sample_only.split(" ")) ## get word count
                        if dataset_utils.extract_answer(sample_only) == eval_batch['ans'][i]:
                            num_correct += 1
                    for prompt, sample, a in zip(eval_batch['prompt'], policy_samples, eval_batch["ans"]):
                        policy_text_table.add_data(self.example_counter, prompt, dataset_utils.extract_sample(sample), a)
                    if self.config.loss.name in {'dpo', 'ipo'}:
                        for prompt, sample, a in zip(eval_batch['prompt'], reference_samples, eval_batch["ans"]):
                            reference_text_table.add_data(self.example_counter, prompt, dataset_utils.extract_sample(sample), a)
                    all_eval_metrics["eval_correct"].append(num_correct/len(policy_samples))
                    all_eval_metrics["word_count"].append(word_count/len(policy_samples))
